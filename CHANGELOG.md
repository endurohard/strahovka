...
## 2026-06-04 - Фикс рассылки: бэклог просроченных страховок

### 🐛 ИСПРАВЛЕНИЯ

После починки очереди отправки (03.06) рассылка выгребла весь исторический
бэклог: 64 из 76 сообщений ушли клиентам с уже истёкшими полисами, с текстом
вида «Осталось всего -16 дней!».

**Причина:** `getDailyReminders` выбирал по `reminder_date <= CURRENT_DATE`
без нижней границы — пока отправка была сломана, у клиентов копился
`last_reminder_sent = NULL`, и после фикса все они стали «due».

**Исправлено:**
- `database.js` → `getDailyReminders`: добавлен фильтр
  `expiration_date::date >= CURRENT_DATE` — напоминания только по активным полисам
- `database.js` → `getStats`: счётчик «Ожидают отправки» не считает просроченных
- `database.js` → `getDueContactsWithoutPhone`: нет уведомлений админу
  о давно истёкших клиентах без номера
- `whatsappPuppeteer.js` → `createReminderMessage`: защитная ветка
  `daysLeft < 0` → «Срок действия страховки уже истёк» (вместо «-16 дней»)

**Известная проблема (не исправлено):** дубли клиентов с одним номером
в разном формате (`+7903…` vs `7903…`) обходят уникальный индекс
`(phone_formatted, start_date)` — клиент id 3019/53734 получил 2 сообщения.
Нужна нормализация номера при импорте.
